...
export const MOCK_THRESHOLDS: ThresholdConfig[] = MOCK_SENSORS.map((s) => ({
  sensor_id: s.id,
  name: s.name,
  mount_height_mm: s.mount_height_mm,
  warning: s.threshold_warning,
  critical: s.threshold_critical,
  warning_mm: Math.round(s.threshold_warning * 1000),
  critical_mm: Math.round(s.threshold_critical * 1000),
}));
